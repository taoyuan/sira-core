...
module.exports = function (Role, app) {
    ...
    var RoleMapping = app.model('RoleMapping');
    ...
    Role.isOwner = function isOwner(modelClass, modelId, userId, callback) {
        ...
        modelClass.findById(modelId, function (err, inst) {
            ...
            if (ownerId) {
                ...
                for (var name in modelClass.relations) {
                    var rel = modelClass.relations[name];
                    if (rel.type === 'belongsTo' && isUserClass(rel.modelTo)) {
                        debug('Checking relation %s to %s: %j', rel, rel.modelTo.modelName, rel);
                        inst[name](function (err, user) {
                            if (!err && user) {
     ...
     * @param {Function} [callback] (err, isAuthenticated)
     */
     ...
     * @param {Function} callback (err, isInRole)
     */
     ...
     * @param {Object} context The security context
     * @param {Function} callback (err, roles)
     */
    Role.getRoles = function (context, callback) {
        ...
        context.principals.forEach(function (p) {
            ...
            if (principalType && principalId) {
                // Please all() treat undefined matches all values
                inRoleTasks.push(function (done) {
                    RoleMapping.all({where: {principalType: principalType,
